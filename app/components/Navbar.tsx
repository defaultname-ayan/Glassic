      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-purple-100 dark:bg-purple-900/50"
            />
          )}
...
export const MobileNavToggle = ({
  ...
  return (
    ...
    >
      {isOpen ? (
        <IconX className="h-6 w-6 text-purple-700 dark:text-purple-300" />
      ) : (
        <IconMenu2 className="h-6 w-6 text-purple-700 dark:text-purple-300" />
      )}
    </button>
  );
};
